...
public class Main1Fragment extends BaseFragment implements
    ...
    protected void initView() {
        ...
        View menu = LayoutInflater.from(getActivity()).inflate(R.layout.main_menu_view, null);
        ...
        mIwHelper.setTranslucentStatus(!isTranslucentStatus ? GetUIDimens.calcStatusBarHeight(getActivity()) : 0);
    }
...
//                -> menuViewPager.dispatchTouchEvent(motionEvent));
    ...
    private void setMainMenu(GenericDrawerLayout genericdrawerlayout, View menu) {
        ...
        if (mViewPager.getCurrentItem() == 0)
            genericdrawerlayout.setTouchSizeOfOpened(Util.dip2px(getActivity(), 150));
    ...
    private void selectImg() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            for (int i = 0; i < PermissionC.WR_FILES_PERMISSION.length; i++) {
                int checkI = getActivity().checkSelfPermission(PermissionC.WR_FILES_PERMISSION[i]);
    ...
    private void showPerDialog(String[] pers) {
        View view = LayoutInflater.from(getActivity()).inflate(R.layout.permission_dialog_bg, null);
        MyDialogUtil.showPermissionAlert(getActivity(),
