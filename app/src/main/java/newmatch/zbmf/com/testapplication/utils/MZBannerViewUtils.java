...
public class MZBannerViewUtils {
     ...
     */
    public static void bannerPageClick(Boolean isShowIndicator, MZBannerView bannerView,
                                       List<Integer> bannerList, BannerViewHolderType bannerViewHolderType) {
        bannerView.setIndicatorVisible(isShowIndicator);
        //传进参赛单位的图片数量
        bannerView.setPages(bannerList, imgs -> {
            BannerViewHolder bannerViewHolder = new BannerViewHolder(imgs,bannerViewHolderType);
            //设置Banner的点击事件
            bannerViewHolder.setBannerClickListener((view, position) -> {
                ToastUtils.showSingleToast(view.getContext(),"点击了Banner："+position);
            });
